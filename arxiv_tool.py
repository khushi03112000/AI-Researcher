#For Web scrapping & web browsing

#Step1: Access arXiv using URL
# going to arXive -> giving info in XML -> returning useful data in JSON
import requests
...
#Step2: Response will get in XML so extracting useful data from it
...
#Step3: Convert the functionality written so far into a tool 

#Note: When agent is created and other agent sees what function is does, it doesn't see its code, rather it sees its docstring, so it is imp that docstring should be PRecise and clear

from langchain_core.tools import tool

#this tool decorator creates a normal python function "arxiv_search" into a TOOL which AI agent can use
@tool
